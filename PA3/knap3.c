...
void solve_kp(int start, long end, long C, long *weights, long *profits, long *solu){
	...
	free(A1);
	free(A2);
	solve_kp(start, mid2, first_half, weights, profits, solu);
	solve_kp(mid2+1, end, C-first_half, weights, profits, solu); 
	return;	
}
...
int main(int argc, char **argv) {
   ...
   FILE   *fp;
   long    N, C, count2;                   // # of objects, capacity 
...
//      if ( table == NULL ) {
  //       printf("[ERROR] : Failed to allocate memory for the whole table.\n");
   ...
   solve_kp(0,N-1,C,weights,profits,solu);
   stop_timer();
   time = elapsed_time ();
   long HA=0;
   for (long m =0; m<N; m++){
