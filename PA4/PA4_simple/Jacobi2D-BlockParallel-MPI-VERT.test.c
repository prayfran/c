...
int main( int argc, char* argv[] ){
  ...
  if((cmdLineArgs.tile_len_x*cmdLineArgs.tile_len_y*p_count <
       cmdLineArgs.problemSize*cmdLineArgs.problemSize )||(cmdLineArgs.tile_len_x != cmdLineArgs.problemSize) || (ceild(cmdLineArgs.problemSize,cmdLineArgs.tile_len_y) != p_count)){
	       if(rank == 0){
		         //fprintf(stderr,"The tile size is too small to accomodate the problem\n");
				 fprintf(stderr,"For Vertical only communication:\nX tile size = problem size.\nNumber of processors = ceild(problem size,Y tile size). \nTry again !!\n");
				             }
  ...
  for( t = 0; t < cmdLineArgs.T; ++t ){ 
	if( rank == 0 ) {
		if(p_count!=1){
	    MPI_Send( space[read][y_ub-1], x_ub, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD );
	    	
		MPI_Recv( space[read][y_ub], x_ub, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &s );	 
		   
		}
    } else if (rank == (p_count-1) ) {
	    MPI_Recv(space[read][y_lb-1], x_ub, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &s );
	    
		MPI_Send(space[read][y_lb], x_ub, MPI_DOUBLE, rank - 1, 0,  MPI_COMM_WORLD );	
