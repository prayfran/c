...
void solve_kp(int start, long end, long C, long *weights, long *profits, long *solu,long depth,long curr){
	...
	free(A1);
	free(A2);
	#pragma omp parallel if (depth==curr)
	solve_kp(start, mid2, first_half, weights, profits, solu,depth,curr+1);
	#pragma omp parallel if (depth==curr)
	solve_kp(mid2+1, end, C-first_half, weights, profits, solu,depth,curr+1); 
	return;	
}
...
//      }
   ...
   solve_kp(0,N-1,C,weights,profits,solu, depth, curr);
