...
#ifdef COUNT
#define COUNT_MACRO_INIT() unsigned long long int kernelCalls = 0,          \
resourceIters = 0, bytesFromGlobal = 0, bytesToGlobal = 0, countTiles = 0,  \
uniqueWavefronts[10]={0}                                                    
#define COUNT_MACRO_KERNEL_CALL() kernelCalls +=1
#define COUNT_MACRO_RESOURCE_ITERS(x) resourceIters += (x)
#define COUNT_MACRO_BYTES_FROM_GLOBAL(x) bytesFromGlobal += (x)
#define COUNT_MACRO_BYTES_TO_GLOBAL(x) bytesToGlobal += (x)
#define COUNT_MACRO_NUMTILES(x) countTiles += (x)
...
#define COUNT_MACRO_PRINT()                                            \
  printf(",KernelCalls:%llu,ResourceIters:%llu,bytesFromGlobal:%llu,\
...
while(uniqueWavefronts[counter]!=0){                                   \
  printf("%llu",uniqueWavefronts[counter]);                            \
 ...
 printf("\n");
  ...
  for( x = lowerBound; x <= upperBound; ++x ){
    for( y = lowerBound; y <= upperBound; ++y ){
      ...
      fprintf(stderr,"Position: %d,%d, values: %a, %a\n",
...
bool verifyResultJacobi2DTiled(double** result,int problemSize, int seed,int steps,
